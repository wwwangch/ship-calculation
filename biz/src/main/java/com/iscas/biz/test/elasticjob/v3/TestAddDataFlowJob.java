...
@Component("testAddDataFlowJob")
public class TestAddDataFlowJob implements DataflowJob<String> {
    @Override
...
//        testCount.set(testCount.get() + 1);
        log.debug("测试动态添加的流式定时任务拉取数据，未配置为流式");
    ...
    public void processData(ShardingContext shardingContext, List<String> data) {
        log.debug("测试动态添加的消费流式数据,未配置为流式，数据为:{}", data.toString());
    }
}
